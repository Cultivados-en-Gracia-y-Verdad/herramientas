#!/usr/bin/env python3
"""Apply Spanish case markers (de· for genitive nouns/adjectives without article)."""
...
from grc_inflect_es import inflect_genitive_mark  # noqa: E402
...
def reapply_file(path: Path) -> int:
    ...
    for i, row in enumerate(rows):
        ...
        new_es = inflect_genitive_mark(str(row.get("es", "")), morph, prev)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Reapply genitive de· markers on N/A glosses.")
    ...
    for book in books:
        ...
        total += n
        if n:
            print(f"{book}: {n} case glosses updated")
