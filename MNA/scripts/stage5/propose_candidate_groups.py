...
VERSION = "stage5-candidate-grouping-prototype-v2"
...
def clear_subject(row: dict[str, Any]) -> str:
    ...
    return f"{explicit} ({pn})" if explicit else pn
...
def make_group(book: str, group_num: int, rows: list[dict[str, Any]], start_boundary: list[str], end_boundary: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "boundary_evidence": {"start": start_boundary, "end": end_boundary},
        "confidence": confidence_for(evidence, len(rows)),
        "status": "REVIEW",
    }


def propose_groups(book: str, rows: list[dict[str, Any]], threshold: int = 4, min_size: int = 2, disruption_streak: int = 2) -> list[dict[str, Any]]:
    if not rows:
        return []
    ...
    for prev, cur in zip(rows, rows[1:]):
        ...
        if pressure >= threshold:
            boundary_streak += 1
            pending_boundary.extend(reasons)
        else:
            boundary_streak = 0
            pending_boundary = []

        if boundary_streak >= disruption_streak and len(current) >= min_size:
            groups.append(make_group(book, group_num, current, start_boundary, pending_boundary))
            group_num += 1
            current = [cur]
            start_boundary = pending_boundary[:]
            pending_boundary = []
            boundary_streak = 0
        else:
            current.append(cur)
...
def md_for_groups(book: str, groups: list[dict[str, Any]]) -> str:
    ...
    out.append("")
    out.append("Stage 5 proposes local grouping candidates from sustained disruption boundaries.")
    out.append("")
    for g in groups:
        ...
        out.append("|---|---|---|---|---|")
        for a in g["anchors"]:
            out.append(f"| {a['ref']} | {a['verb']} | {a['subject']} | {a['movement']} | {' '.join(a['connectors']) or '-'} |")
        out.append("")
    return "\n".join(out)
...
def main() -> int:
    ...
    ap.add_argument("--disruption-streak", type=int, default=2)
    args = ap.parse_args()
    ...
    groups = propose_groups(book, rows, threshold=args.threshold, min_size=args.min_size, disruption_streak=args.disruption_streak)
    ...
    metadata = {
        ...
        "disruption_streak": args.disruption_streak,
    ...
    audit_path.write_text(json.dumps({
        ...
        "min_size": args.min_size,
        "disruption_streak": args.disruption_streak,
        "outputs": {"jsonl": str(jsonl_path), "markdown": str(md_path)},
