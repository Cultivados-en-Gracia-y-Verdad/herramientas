#!/usr/bin/env python3
"""
MNA Stage 4 — absolute dependency candidate detector.

PURPOSE
- Read Stage 3 anchor skeleton rows.
- Read MorphGNT source tokens for local context.
- Detect high-confidence grammatical dependency candidates for manual audit.
...
PC-DEP-CAND-SUBJ-001:
- finite predicate anchor is subjunctive,
- a recognized subordinating trigger appears in a local pre-anchor window,
- the result is recorded as a dependency candidate, not as final classification.
...
def token_is_subjunctive_dependency_trigger(token: SourceToken) -> bool:
    candidates = {
        token.greek_clean,
        token.lemma_clean,
    ...
    return bool(candidates & SUBJUNCTIVE_DEPENDENCY_TRIGGERS)


def find_local_subjunctive_trigger(
    row: dict[str, object],
    tokens_by_ref: dict[tuple[int, int], list[SourceToken]],
    window: int,
) -> Optional[SourceToken]:
    mood_code = str(row.get("mood_code", ""))
    mood = str(row.get("mood", ""))

    if mood_code != "S" and mood != "subjunctive":
        return None
    ...
    for token in reversed(local_tokens):
        if token_is_subjunctive_dependency_trigger(token):
            return token

    return None


def build_candidate_record(row: dict[str, object], trigger: SourceToken) -> dict[str, object]:
    return {
        "record_type": "absolute_dependency_candidate",
        "candidate_status": "DEPENDENCY_CANDIDATE_FOR_MANUAL_AUDIT",
        "rule_family": "PC-DEP-CAND-SUBJ",
        "rule_id": "PC-DEP-CAND-SUBJ-001",
        "reason": "Subjunctive finite predicate anchor has a recognized local subordinating trigger in the pre-anchor window.",
        "connector_dependency_used": "NO_OFFICIAL_CLASSIFICATION_NOT_CHANGED",
        "book": row["book"],
        "chapter": row["chapter"],
        "verse": row["verse"],
        ...
        "trigger_source_line_number": trigger.source_line_number,
...
def detect_candidates(
    ...
    for row in rows:
        trigger = find_local_subjunctive_trigger(row, tokens_by_ref, window)
        if trigger is not None:
            candidates.append(build_candidate_record(row, trigger))

    output_path.parent.mkdir(parents=True, exist_ok=True)

    metadata = {
        "record_type": "metadata",
        "stage": "Stage 4 — Absolute Dependency Candidate Audit",
        "producer_script": "scripts/stage4/detect_absolute_dependency_candidates.py",
        "producer_command": f"python3 scripts/stage4/detect_absolute_dependency_candidates.py {book}",
        "version": VERSION,
        "book": book,
...
def print_visible_output(
    ...
    print("MNA Stage 4 — Absolute Dependency Candidate Detector")
    print(f"BOOK: {book}")
    ...
    print(f"DEPENDENCY CANDIDATES FOUND: {metadata['dependency_candidates_found']}")
    ...
    for idx, candidate in enumerate(candidates[:preview_lines], start=1):
        print(
            ...
            f"trigger={candidate['trigger_greek_surface']} | "
    ...
    if remaining:
        print(f"... {remaining} more dependency candidates written")


def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Detect Stage 4 high-confidence dependency candidates for manual audit."
    )
    ...
    try:
        ...
        output_path = root / "audits" / "stage4" / "absolute-dependency-candidates" / f"{book}.jsonl"
        ...
        print("MNA Stage 4 dependency candidate detection FAILED", file=sys.stderr)
