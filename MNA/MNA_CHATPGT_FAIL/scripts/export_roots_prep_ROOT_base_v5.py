    ...
    with path.open(encoding="utf-8") as f:
        for lineno, line in enumerate(f, start=1):
            ...
            if "\t" in line:
                ...
                # Some older token files may be space-separated instead of tab-separated.
                # Accept: 01 token
                # Reject malformed lines with a useful message.
                parts = line.split(maxsplit=1)

                if len(parts) != 2 or not parts[0].isdigit():
                    raise ValueError(
                        f"Malformed token line in {path} at line {lineno}: {line!r}. "
                        "Expected IDX<TAB>TEXT or IDX TEXT."
                    )
            ...
            if not idx.isdigit():
                raise ValueError(
                    f"Malformed token index in {path} at line {lineno}: {idx!r}"
                )

            rows.append((idx.zfill(2), text))
...
def leading_contiguous_indexes(indexes: list[str]) -> list[str]:
    """
    Keep only the first contiguous NBLA span.

    This prevents one Greek finite verb from swallowing later implied Spanish
    wording when an alignment uses non-contiguous NBLA indexes.
    ...
    nums = sorted(int(x) for x in indexes if str(x).isdigit())
    ...
    return [f"{n:02d}" for n in kept]


def span_text_from_spanish_tokens(
    ...
    for idx, greek in greek_tokens:
        ...
        if morph and is_verb(morph["code"]):
            ...
            if is_finite(code):
                ...
                primary_indexes = leading_contiguous_indexes(indexes)

                if spanish != "[sin equivalente]":
                    primary_spanish = span_text_from_spanish_tokens(
                        spanish_tokens,
                        primary_indexes,
                        spanish,
                    )
                    display = f"=={primary_spanish}=="
                else:
                    display = f"[=={morph['lemma']}==]"

                if primary_indexes:
                    verb_marks[primary_indexes[0]] = display
                    hidden_indexes.update(primary_indexes[1:])
            else:
