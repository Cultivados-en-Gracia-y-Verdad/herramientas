#!/usr/bin/env python3
"""
MNA Stage 4 — content-clause dependency candidate detector.

PURPOSE
- Read Stage 3 anchor skeleton rows.
- Read MorphGNT source tokens for local context.
- Detect finite predicate anchors inside narrow content-clause environments.
...
PC-DEP-CAND-CONTENT-001:
- a narrow speech/cognition/reporting predicate appears locally before ὅτι,
- ὅτι appears between that governing predicate and the target finite predicate anchor,
- no intervening finite predicate appears between ὅτι and the target anchor,
...
VERSION = "stage4-content-clause-dependency-candidates-v1"
...
# Deliberately small first-pass list.
# These are lemmas, not English semantics.
GOVERNING_LEMMAS = {
    "λέγω",
    "οἶδα",
    "γινώσκω",
...
def is_content_marker(token: SourceToken) -> bool:
    candidates = {
        token.greek_clean,
        token.lemma_clean,
    ...
    return bool(candidates & CONTENT_MARKERS)


def is_governing_predicate(token: SourceToken) -> bool:
    if not is_finite_source_token(token):
        return False
    return token.lemma_clean in GOVERNING_LEMMAS


def find_content_environment(
    row: dict[str, object],
    tokens_by_ref: dict[tuple[int, int], list[SourceToken]],
    window: int,
) -> Optional[tuple[SourceToken, SourceToken]]:
    ...
    # Search from nearest ὅτι back to a governing predicate.
    for content_marker in reversed(local_tokens):
        if not is_content_marker(content_marker):
            continue

        between_marker_and_anchor = [
            tok
            for tok in tokens
            if content_marker.token_index_in_verse < tok.token_index_in_verse < anchor_index
        ]
        if any(is_finite_source_token(tok) for tok in between_marker_and_anchor):
            continue

        before_marker = [
            tok
            for tok in local_tokens
            if tok.token_index_in_verse < content_marker.token_index_in_verse
        ]

        for governor in reversed(before_marker):
            if not is_governing_predicate(governor):
                continue

            between_governor_and_marker = [
                tok
                for tok in tokens
                if governor.token_index_in_verse < tok.token_index_in_verse < content_marker.token_index_in_verse
            ]
            if any(is_finite_source_token(tok) for tok in between_governor_and_marker):
                continue

            return governor, content_marker

    return None


def build_candidate_record(row: dict[str, object], governor: SourceToken, content_marker: SourceToken) -> dict[str, object]:
    return {
        "record_type": "content_clause_dependency_candidate",
        "candidate_status": "DEPENDENCY_CANDIDATE_FOR_MANUAL_AUDIT",
        "rule_family": "PC-DEP-CAND-CONTENT",
        "rule_id": "PC-DEP-CAND-CONTENT-001",
        "reason": "Finite predicate anchor follows local ὅτι content marker governed by a narrow speech/cognition/reporting predicate, with no intervening finite predicate after ὅτι.",
        ...
        "content_marker_token_index_in_verse": content_marker.token_index_in_verse,
        "content_marker_greek_surface": content_marker.greek,
        "content_marker_lemma": content_marker.lemma,
    }
...
def detect_candidates(
    ...
    for row in rows:
        content_env = find_content_environment(row, tokens_by_ref, window)
        if content_env is None:
            continue
        governor, content_marker = content_env
        candidates.append(build_candidate_record(row, governor, content_marker))

    output_path.parent.mkdir(parents=True, exist_ok=True)

    metadata = {
        "record_type": "metadata",
        "stage": "Stage 4 — Content Clause Dependency Candidate Audit",
        "producer_script": "scripts/stage4/detect_content_clause_dependency_candidates.py",
        "producer_command": f"python3 scripts/stage4/detect_content_clause_dependency_candidates.py {book}",
        "version": VERSION,
        "book": book,
...
def print_visible_output(
    ...
    print("MNA Stage 4 — Content Clause Dependency Candidate Detector")
    print(f"BOOK: {book}")
    ...
    print(f"CONTENT CLAUSE DEPENDENCY CANDIDATES FOUND: {metadata['dependency_candidates_found']}")
    ...
    for idx, candidate in enumerate(candidates[:preview_lines], start=1):
        print(
            ...
            f"governor={candidate['governor_greek_surface']} | "
            f"marker={candidate['content_marker_greek_surface']} | "
    ...
    if remaining:
        print(f"... {remaining} more content-clause dependency candidates written")


def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Detect Stage 4 content-clause dependency candidates for manual audit."
    )
    parser.add_argument("book", help="Book slug, e.g. 1corintios")
    parser.add_argument("--source", help="Explicit MorphGNT source path")
    parser.add_argument("--window", type=int, default=12, help="Number of previous same-verse tokens to inspect")
    ...
    try:
        ...
        output_path = root / "audits" / "stage4" / "content-clause-dependency-candidates" / f"{book}.jsonl"
        ...
        print("MNA Stage 4 content-clause dependency candidate detection FAILED", file=sys.stderr)
