#!/usr/bin/env python3
"""
MNA Stage 4 — unresolved independent candidate renderer.

PURPOSE
- Read datasets/independent-clause-candidates/<book>.jsonl.
- Print only rows still marked UNRESOLVED_CANDIDATE.
- Help identify obvious remaining dependency patterns by inspection.
...
It is only a diagnostic renderer.
"""
...
VERSION = "stage4-unresolved-independent-candidate-renderer-v1"
...
def context_for_row(
    ...
    # token_index_in_verse is preserved in Stage 3, but not carried into this dataset.
    # Recover it safely from the predicate_anchor_id shape: book-ch-v-pa-token-anchorOrder.
    match = re.search(r"-pa-(\d+)-\d+$", str(row["predicate_anchor_id"]))
    if not match:
        return ""
...
def render_rows(
    ...
    for display_index, row in enumerate(selected, start=start):
        print(
            ...
            f"mood={row['mood']} | morph={row['morphology']}"
        )
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Render unresolved Stage 4 independent-clause candidates."
    )
    ...
    try:
        ...
        unresolved = [
            row for row in rows
            if row.get("independent_clause_candidate") == UNRESOLVED_STATUS
        ]
        ...
        print("MNA Stage 4 — Unresolved Independent Candidate Render")
        print(f"BOOK: {book}")
        print(f"VERSION: {VERSION}")
        print(f"DATASET: {dataset_path}")
        print(f"TOTAL ROWS: {len(rows)}")
        print(f"UNRESOLVED_CANDIDATES: {len(unresolved)}")
        ...
        remaining = len(unresolved) - (max(1, args.start) - 1 + args.limit)
        if remaining > 0:
            print(f"... {remaining} more unresolved candidates not shown")

        return 0
    except Exception as exc:
        print("MNA Stage 4 unresolved independent candidate render FAILED", file=sys.stderr)
