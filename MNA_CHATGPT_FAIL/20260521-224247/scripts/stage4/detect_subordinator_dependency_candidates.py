#!/usr/bin/env python3
"""
MNA Stage 4 — subordinator dependency candidate detector.

PURPOSE
- Read Stage 3 anchor skeleton rows.
- Read MorphGNT source tokens for local context.
- Detect finite predicate anchors introduced by clear subordinating markers.
...
PC-DEP-CAND-SUBORD-001:
- a clear subordinating marker appears in a local pre-anchor window,
- the finite predicate anchor follows that marker in the same verse,
- no intervening finite predicate appears between the marker and the anchor,
- the result is recorded as a dependency candidate for manual audit.

This detector intentionally avoids ordinary coordinating conjunctions such as καί, δέ, γάρ, ἀλλά.
"""
...
VERSION = "stage4-subordinator-dependency-candidates-v1"
...
# Clear subordinating markers only. Coordinators are intentionally excluded.
# The detector normalizes accents/breathing, so unaccented backups are included for clarity.
...
def is_subordinating_marker(token: SourceToken) -> bool:
    candidates = {
        token.greek_clean,
        token.lemma_clean,
    ...
    return bool(candidates & SUBORDINATING_MARKERS)


def find_local_subordinator(
    row: dict[str, object],
    tokens_by_ref: dict[tuple[int, int], list[SourceToken]],
    window: int,
    ...
    for candidate in reversed(local_tokens):
        if not is_subordinating_marker(candidate):
            continue
...
def build_candidate_record(row: dict[str, object], subordinator: SourceToken) -> dict[str, object]:
    return {
        "record_type": "subordinator_dependency_candidate",
        "candidate_status": "DEPENDENCY_CANDIDATE_FOR_MANUAL_AUDIT",
        "rule_family": "PC-DEP-CAND-SUBORD",
        "rule_id": "PC-DEP-CAND-SUBORD-001",
        "reason": "Finite predicate anchor follows a local subordinating marker with no intervening finite predicate in the same verse.",
        ...
        "subordinator_token_index_in_verse": subordinator.token_index_in_verse,
        "subordinator_greek_surface": subordinator.greek,
        "subordinator_lemma": subordinator.lemma,
        "subordinator_pos": subordinator.pos,
        "subordinator_parsing": subordinator.parsing,
    }
...
def detect_candidates(
    ...
    for row in rows:
        subordinator = find_local_subordinator(row, tokens_by_ref, window)
        if subordinator is not None:
            candidates.append(build_candidate_record(row, subordinator))

    output_path.parent.mkdir(parents=True, exist_ok=True)

    metadata = {
        "record_type": "metadata",
        "stage": "Stage 4 — Subordinator Dependency Candidate Audit",
        "producer_script": "scripts/stage4/detect_subordinator_dependency_candidates.py",
        "producer_command": f"python3 scripts/stage4/detect_subordinator_dependency_candidates.py {book}",
        "version": VERSION,
        "book": book,
...
def print_visible_output(
    ...
    print("MNA Stage 4 — Subordinator Dependency Candidate Detector")
    print(f"BOOK: {book}")
    ...
    print(f"SUBORDINATOR DEPENDENCY CANDIDATES FOUND: {metadata['dependency_candidates_found']}")
    ...
    for idx, candidate in enumerate(candidates[:preview_lines], start=1):
        print(
            ...
            f"subordinator={candidate['subordinator_greek_surface']} | "
    ...
    if remaining:
        print(f"... {remaining} more subordinator dependency candidates written")


def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Detect Stage 4 subordinator dependency candidates for manual audit."
    )
    ...
    try:
        ...
        output_path = root / "audits" / "stage4" / "subordinator-dependency-candidates" / f"{book}.jsonl"
        ...
        print("MNA Stage 4 subordinator dependency candidate detection FAILED", file=sys.stderr)
