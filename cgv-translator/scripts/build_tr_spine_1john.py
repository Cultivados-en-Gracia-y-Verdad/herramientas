#!/usr/bin/env python3
"""Build 1 John TR spine from Robinson-parsed Scrivener (UTR) + remap phrases.

Primary textual/morph source:
  Biblia-LBF/source/greek/TR1894/robinson-parsed/1JO.UTR
...
UTR_PATH = HERR / "Biblia-LBF/source/greek/TR1894/robinson-parsed/1JO.UTR"
SCV_PATH = HERR / "Biblia-LBF/source/greek/TR1894/scrivener-textonly/1JO.SCV"
TR_ACCENT_PATH = HERR / "Biblia-LBF/source/greek/TR1894/tr1894.txt"
MORPH = HERR / "MNA/SOURCES/MorphGNT/83-1Jn-morphgnt.txt"
PHRASES = HERR / "cgv-translator/translations/1john-phrases.json"
OUT_DIR = HERR / "cgv-translator/translations/tr-spine/1john"
BOOK_CODE = 62
...
def main() -> None:
    ...
    (OUT_DIR / "1john-tr-spine.json").write_text(
    ...
    (OUT_DIR / "1john-phrases-tr.json").write_text(
    ...
    (OUT_DIR / "1john-tr-diff-report.md").write_text(
        "\n".join(lines).rstrip() + "\n", encoding="utf-8"
    )
    (OUT_DIR / "1john-phrase-remap-issues.json").write_text(
